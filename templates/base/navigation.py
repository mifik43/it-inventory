...
def create_knowledge_base_menu():
    ...
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-journal-text", name="Статьи", url="articles.articles_list",
    ...
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-sticky", name="Заметки", url="notes.notes_list",
...
def create_administration_menu():
    ...
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-people", name="Пользователи", url="users.users",
    ...
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-calendar-week", name="Роли", url="roles.roles",
    ...
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-key", name="Пароли", url="password_manager.folders",
    ...
    menu.add_item(MenuItem(button_class="dropdown-item", icon="fa-terminal", name="Скрипты", url="script.script_list",
        urls_to_be_active=['script.script_list'],
        permissions=[]))  # пока без прав, но оставим здесь
    return menu

def create_security_menu():
    """Безопасность (только для администраторов или специалистов по безопасности)"""
    menu = DropDownMenu(name="Безопасность", icon="bi-shield-check")
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-shield-lock", name="Сканирование безопасности", url="security_scan.index",
    ...
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-list-check", name="Whitelist подсетей", url="security_scan.whitelist",
...
def create_organizations_menu():
    """Организации (административный раздел)"""
    menu = DropDownMenu(name="Организации", icon="bi-building")
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-building", name="Список организаций", url="organizations.organizations",
    ...
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-diagram-2", name="Иерархия", url="organizations.hierarchy",
        urls_to_be_active=['organizations.hierarchy'], permissions=[Permissions.organizations_manage]))
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-people", name="Пользователи по организациям", url="organizations.users_by_organization",
        urls_to_be_active=['organizations.users_by_organization'], permissions=[Permissions.organizations_manage]))
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-bar-chart", name="Отчёт по активности", url="organizations.activity_report",
        urls_to_be_active=['organizations.activity_report'], permissions=[Permissions.organizations_manage]))
    return menu

# ========== ТЕСТОВЫЕ РАЗДЕЛЫ (скрыты для обычных пользователей) ==========

def create_social_menu():
    """Соцсети (тестовый функционал)"""
    menu = DropDownMenu(name="Соцсети (тест)", icon="bi-share")
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-clock-history", name="История", url="social.social_history",
        urls_to_be_active=['social.social_history'], permissions=[Permissions.social_read, Permissions.social_manage]))
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-calendar-event", name="Запланированные", url="social.scheduled_posts",
        urls_to_be_active=['social.scheduled_posts'], permissions=[Permissions.social_read, Permissions.social_manage]))
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-gear", name="Настройки платформ", url="social.social_platforms",
        urls_to_be_active=['social.social_platforms'], permissions=[Permissions.social_manage]))
    return menu

def create_checklist_menu():
    """Чек-лист (можно оставить в тестовых)"""
    menu = DropDownMenu(name="Чек-лист (тест)", icon="bi-clipboard-check")
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-clipboard-check", name="Чек-лист", url="checklist.checklist",
        urls_to_be_active=['checklist.checklist','checklist.add_checklist_task','checklist.edit_checklist_task'],
        permissions=[]))
    menu.add_item(MenuItem(button_class="dropdown-item", icon="bi-graph-up", name="Статистика", url="checklist.checklist_stats",
        urls_to_be_active=['checklist.checklist_stats'], permissions=[]))
    return menu
...
def create_main_menu():
    ...
    menu.add_item(create_operations_menu())          # Рабочие инструменты
    menu.add_item(create_knowledge_base_menu())      # База знаний
    menu.add_item(create_scanning_menu())            # Сканирование сети
    menu.add_item(create_organizations_menu())       # Организации (админ)
    menu.add_item(create_hr_menu())                  # Кадровое (HR)
    menu.add_item(create_administration_menu())      # Администрирование
    menu.add_item(create_security_menu())            # Безопасность (админ)
    menu.add_item(create_social_menu())              # Соцсети (тест)
    menu.add_item(create_checklist_menu())           # Чек-лист (тест)
    return menu